...
<div class="tem1">
  ...
  <table class="table">
    <tr>
      <td class="td1"><input class="input" type="text" v-model="group"><button class="button-64 b1" @click="addGroup">Add Group</button></td>
      <td>
        <p>Sort :</p>
    ...
    <table>
      <tr>
        <td><h3>{{group.groupName}}</h3></td>
        <td><button class="button-64 button-65" @click="removeGroup(group)">Delete</button></td>
      </tr>
       <tr>
    ...
    <tr v-for="row in group.rows" :key="row" >
      ...
      <td><input class="input" id="priority" @click="changePriority(row)" readonly type="text" v-model="row.priority"></td>
      <td><input class="input" @click="changeState(row)" readonly type="text"  v-model="row.state"></td>
      <td><button class="button-64 button-65" @click="removeRow(group,row)">Done</button></td>
      <td><input type="text" style="visibility:hidden" v-model="row.id"></td>
    </tr>
    </table>
    <div>
  <button class="button-64 button-65" @click="addRow(group)">Add Task</button>
</div> 
    </td>
    </tr>
</table>
...
methods:{
   addRow: function(group){
    ...
    removeRow: function(group,row){
      var index = group.rows.indexOf(row);
      group.rows.splice(index, 1);
      
       axios.delete(`http://127.0.0.1:8000/api/v1/mods/${row.id}`).then((response) => { 
      response.data;
      alert("deleted id : " + row.id);
       })
    },
    addGroup: function(){
      if(this.group === 'undefined' || this.group == null || this.group.replace(/\s/g, '').length < 1)
      {
        alert('e-e');
        return;
      }
        this.groups.push({groupName:this.group, rows:[{task:"",priority:""}]});
    },
    removeGroup: function(group){
      for(var i = 0; i < group.rows.length; i++){
      axios.delete(`http://127.0.0.1:8000/api/v1/mods/${group.rows[i].id}`).then((response) => { 
      response.data;
       })}
      ...
      axios.get('http://127.0.0.1:8000/api/v1/mods/').then((response) => {
        response.data;
          ...
          for(var i = 0; i < response.data.length; i++){
            ...
            if(found){
            this.groups[j].rows.push({task:response.data[i].task, priority: response.data[i].priority, state: response.data[i].state, id: response.data[i].id});
            }
            else{
              this.groups.push({groupName:response.data[i].groupName, rows:[{task:response.data[i].task, priority: response.data[i].priority, state: response.data[i].state, id: response.data[i].id}]});
            }
 ...
 postList(){
   for(var i = 0; i < this.groups.length ; i++ ){
    ...
    axios.put(`http://127.0.0.1:8000/api/v1/mods/${this.groups[i].rows[j].id}`, {groupName: groupName, task: task.task, state: task.state, priority:task.priority}).then((response) => { 
      response.data;
    })
    }
      else{
      axios.post('http://127.0.0.1:8000/api/v1/mods/', {groupName: groupName, task: task.task, state: task.state, priority:task.priority}).then((response) => { 
       response.data;
    })
      }
     }
